  ...
  internal class ProcessMemory {
    ...
    public IntPtr GetModuleBaseAddress(string ModuleName) {
      ...
      for (int index = 0; index < modules.Count; ++index) {
        ProcessModule processModule = modules[index];
        if (!processModule.ModuleName.Equals(ModuleName)) continue;
        num = processModule.BaseAddress;
        break;
      }
    ...
    public IntPtr FindPattern(byte[] bytePattern, string stringMask, IntPtr startAddress, int size) {
      ...
      byte[] data = ReadBytes(startAddress, size);

      for (int x = 0; x < size; x++) {
        if (MaskCheck(data, x, bytePattern, stringMask)) {
          return new IntPtr(startAddress.ToInt32() + x);
        }
      }
